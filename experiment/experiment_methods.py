...
def fit_pow_ndamped_meas(measurement, x_var, y_vars, n_damped, n_over_damped, p0=None, freq_bounds=None, bounds=None, normalize=True, kwargs_dict={}):
    ...
    for ii, y_var in enumerate(y_vars):
        ...
        for jj, f in enumerate(freqs):
            if jj<n_damped:
                '''
                mode_params = [f, damps[jj], d_amps[ii][jj], 0, 'under damped']
                mode_var_params = [var_freqs[jj], var_damps[jj], var_d_amps[ii][jj], 0, 'under damped']
                data_vars[f'Mode {jj+1} Params ({y_var})'] = (('Mode Params'), mode_params)
                data_vars[f'Mode {jj+1} Params Variance ({y_var})'] = (('Mode Params'), mode_var_params)
                '''
                ...
                '''
                mode_params = [f, damps[jj-n_damped], od_amps[ii][jj-n_damped], od_phis[ii][jj-n_damped], 'under damped']
                mode_var_params = [var_freqs[jj-n_damped], var_damps[jj-n_damped], var_od_amps[ii][jj-n_damped], var_od_phis[ii][jj-n_damped], 'over damped']
                data_vars[f'Mode {jj+1} Params ({y_var})'] = (('Mode Params'), mode_params)
                data_vars[f'Mode {jj+1} Params Variance ({y_var})'] = (('Mode Params'), mode_var_params)
                '''
...
##############################################
### Damped oscillator fits, power spectrum ###
##############################################
...
def fit_pow_ndamped_withc(x, ys, n_damped, n_over_damped, params0=None, bounds=None, freq_bounds=None, normalize=False, kwargs_dict={}):
    '''
    fits for power spectrum (ie, ignores phase information)
    given xdata and a set of y datas, simulatneously fits the ys to same set of frequencies with other paramters variable.
    params are in the form [freqs, c1, other_params1, ..., cn, other_params2]
    ...
    if params0==None:
        ...
        od_amps0 = np.array([[np.real(np.max(ys[j])) for i in range(n_over_damped)] for j in range(n_sets)])
        od_amps20 = np.array([[np.real(np.max(ys[j])) for i in range(n_over_damped)] for j in range(n_sets)])
        params0=[freqs0, damps0, cs0, d_amps0, od_amps0, od_amps20]
    if bounds==None:
        if freq_bounds==None:
            upper_freqs = np.array([np.inf for i in range(n_freqs)])
            lower_freqs = np.array([0 for i in range(n_freqs)])
        ...
        upper_od_amps = np.array([[np.inf for i in range(n_over_damped)] for j in range(n_sets)])
        upper_od_amps2 = np.array([[np.inf for i in range(n_over_damped)] for j in range(n_sets)])
        upper_bounds=flatten_params_pow([upper_freqs, upper_damps, upper_cs, upper_d_amps, upper_od_amps, upper_od_amps2])
        lower_damps = np.array([0 for i in range(n_freqs)])
        lower_cs = np.array([-np.inf]*n_sets)
        lower_d_amps = np.array([[-np.inf for i in range(n_damped)] for j in range(n_sets)])
        lower_od_amps = np.array([[-np.inf for i in range(n_over_damped)] for j in range(n_sets)])
        lower_od_amps2 = np.array([[-np.inf for i in range(n_over_damped)] for j in range(n_sets)])
        lower_bounds=flatten_params_pow([lower_freqs, lower_damps, lower_cs, lower_d_amps, lower_od_amps, lower_od_amps2])
        bounds = (lower_bounds, upper_bounds)
    ...
    if normalize==True:
        opt_params = unflatten_params_pow(opt_params_flattened, n_damped, n_over_damped, n_sets)
        var = unflatten_params_pow(var_flattened, n_damped, n_over_damped, n_sets)
        freqs, damps, cs, d_amps, od_amps, od_phis = opt_params
        var_freqs, var_damps, var_cs, var_d_amps, var_od_amps, var_od_phis = var
        cs = np.array([cs[i]*norm_ys[i] for i in range(n_sets)])
        d_amps = np.array([d_amps[i]*norm_ys[i] for i in range(n_sets)])
        od_amps = np.array([od_amps[i]*norm_ys[i] for i in range(n_sets)])
        var_cs = np.array([var_cs[i]*norm_ys[i] for i in range(n_sets)])
        var_d_amps = np.array([var_d_amps[i]*norm_ys[i] for i in range(n_sets)])
        var_od_amps = np.array([var_od_amps[i]*norm_ys[i] for i in range(n_sets)])
        opt_params = [freqs, damps, cs, d_amps, od_amps, od_phis]
        var = [var_freqs, var_damps, var_cs, var_d_amps, var_od_amps, var_od_phis]
        opt_params_flattened = flatten_params_pow(opt_params)
        var_flattened = flatten_params_pow(var)
...
def flatten_params_pow_withc(params):
    '''
    take parameters made for fit and pack them for residual function. params are in the form
    ...
    freqs, damps, cs, d_amps, od_amps, od_phis = params
    n_sets = len(cs)
    n_damped = len(d_amps[0])
    n_over_damped = len(od_amps[0])
    flattened_params = [f for f in freqs]+[d for d in damps]
...
def pack_params_pow_withc(params, n_d, n_od, n_sets):
    '''
    take flattened parameters made for fit and pack them for residual function. params are in the form
    ...
    packed_params = np.zeros(nparams)
    for i in range(n_sets):
        packed_params[i*nsetparams] = params[2*n_freqs+i*(1+n_d+2*n_od)] # c
        for j in range(n_d):
            packed_params[i*nsetparams+3*j+1] = params[j]
            packed_params[i*nsetparams+3*j+2] = params[n_freqs+j]
            packed_params[i*nsetparams+3*j+3] = params[2*n_freqs+i*(1+n_d+2*n_od)+j+1]
        for j in range(n_od):
            packed_params[i*(nsetparams+3*n_d)+4*j+1] = params[j]
            packed_params[i*(nsetparams+3*n_d)+4*j+2] = params[n_freqs+j]
            packed_params[i*(nsetparams+3*n_d)+4*j+3] = params[2*n_freqs+i*(1+n_d+2*n_od)+2*j+1]
            packed_params[i*(nsetparams+3*n_d)+4*j+4] = params[2*n_freqs+i*(1+n_d+2*n_od)+2*j+2]
    return np.array(packed_params)

def unflatten_params_pow_withc(params, n_damped, n_over_damped, n_sets):
    '''
    unpacks parameters back into original form, ie from 
    ...
    freqs = params[:n_freqs]
    damps = params[n_freqs:2*n_freqs]
    cs = [params[2*n_freqs+i*(1+n_damped + 2*n_over_damped)] for i in range(n_sets)]
    d_amps = [[params[2*n_freqs+i*(1+n_damped + 2*n_over_damped)+1+j] for j in range(n_damped)] for i in range(n_sets)]
    od_amps = [[params[2*n_freqs+i*(1+n_damped + 2*n_over_damped)+1+n_damped+2*j] for j in range(n_over_damped)] for i in range(n_sets)]
    od_phis = [[params[2*n_freqs+i*(1+n_damped + 2*n_over_damped)+1+n_damped+2*j+1] for j in range(n_over_damped)] for i in range(n_sets)]
    unflattened_params = [np.array(freqs), np.array(damps), np.array(cs), np.array(d_amps), np.array(od_amps), np.array(od_phis)]
    return unflattened_params

def residual_pow_ndamped_withc(params, x, ys, fosc, n_damped, n_over_damped, n_sets, n_params):
    '''
    computes residual for ndamped oscillators and 
    '''
    params = pack_params_pow(params, n_damped, n_over_damped, n_sets)
